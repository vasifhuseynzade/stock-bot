...
class StrategyConfig:
    # Defaults are v2.2. The v2.1b baseline is built explicitly in make_variants().
    name: str = "v2_2_rs_breakout_trend"
    ...
    min_score: int = 72
    breakout_min_score: int = 80
    pullback_min_score: int = 999
    weak_min_score: int = 86
    max_signals_per_scan: int = 4
    ...
    # Setup behavior.
    allow_pullbacks: bool = False
    breakout_min_volume_ratio: float = 1.25
    pullback_min_volume_ratio: float = 0.30
    breakout_max_rsi: float = 80.0
    breakout_max_daily_move_pct: float = 6.5
    ...
    # V2.2 breakout-quality gates.
    breakout_min_close_location: float = 0.65
    breakout_min_rs_score: int = 16
    breakout_require_ma100: bool = True
    breakout_require_ma20_slope: bool = True
    breakout_min_stock_ret_20: float = 0.00
    breakout_min_stock_ret_63: float = 0.00
    breakout_min_rel_20_spy: float = 0.00
    breakout_min_rel_63_spy: float = -0.02
    breakout_min_rel_20_qqq: float = -0.03
    breakout_require_55day_or_strong_rs: bool = True
    breakout_strong_rs_20_spy: float = 0.06
    breakout_max_base_range_pct: float = 0.35
    breakout_max_extension_ma20: float = 0.12
    breakout_max_extension_atr: float = 1.60

    # Stop model.
    breakout_atr_stop_mult: float = 2.00
    pullback_atr_stop_mult: float = 1.80
    stop_wider_of_atr_and_structure: bool = True

    # Exit behavior / winner capture.
    breakeven_r: float = 1.10
    partial_r: float = 1.80
    partial_pct: float = 0.12
    partial_fraction: float = 0.33
    trail_mult_before_5pct: float = 3.40
    trail_mult_after_5pct: float = 2.80
    trail_tighten_r: float = 2.0
    trail_tighten_pct: float = 0.12
    enable_time_stop: bool = True
    time_stop_days: int = 7
    time_stop_min_r: float = 0.25
    enable_failed_breakout_exit: bool = True
...
def analyze_signal(ticker: str, day: date, data: Dict[str, pd.DataFrame], market: str, cfg: StrategyConfig) -> Optional[Dict[str, Any]]:
    ...
    if breakout_20 or breakout_55:
        ...
        if volume_ratio is not None and volume_ratio >= 1.5:
            breakout_score += 5
        if close_loc >= cfg.breakout_min_close_location:
            breakout_score += 8
        ...
        # V2.2 hard gates: buy only clean, liquid, leadership breakouts.
        if rsi_val > cfg.breakout_max_rsi:
            breakout_score = -999
        if daily_move_pct <= 0 or daily_move_pct > cfg.breakout_max_daily_move_pct:
            breakout_score = -999
        ...
        if not (price > ma20 > ma50):
            breakout_score = -999
        if cfg.breakout_require_ma100 and (ma100 is None or price <= ma100):
            breakout_score = -999
        if cfg.breakout_require_ma20_slope and not rolling_slope_positive_on_day(df, day, "MA20", 5):
            breakout_score = -999
        if stock_ret_20 is None or stock_ret_20 < cfg.breakout_min_stock_ret_20:
            breakout_score = -999
        if stock_ret_63 is None or stock_ret_63 < cfg.breakout_min_stock_ret_63:
            breakout_score = -999
        if rel_20 is None or rel_20 < cfg.breakout_min_rel_20_spy:
            breakout_score = -999
        if rel_63 is None or rel_63 < cfg.breakout_min_rel_63_spy:
            breakout_score = -999
        if rel_qqq_20 is None or rel_qqq_20 < cfg.breakout_min_rel_20_qqq:
            breakout_score = -999
        if cfg.breakout_require_55day_or_strong_rs:
            ...
            if not (breakout_55 or strong_rs):
                breakout_score = -999
        if recent_range_20_pct > cfg.breakout_max_base_range_pct:
            breakout_score = -999
        if pullback_distance_ma20 > cfg.breakout_max_extension_ma20:
            breakout_score = -999
        if breakout_extension_atr > cfg.breakout_max_extension_atr:
            breakout_score = -999
        if rs_score < cfg.breakout_min_rs_score:
            breakout_score = -999

    pullback_score = -999
    pullback_candidate = False
    if cfg.allow_pullbacks:
        pullback_candidate = (
            price > ma50
            and ma20 >= ma50 * 0.995
    ...
    if setup_type == "breakout":
        ...
        stop_model = "v2_2_breakout_wider_structure_atr" if cfg.stop_wider_of_atr_and_structure else "v2_2_breakout_tighter_structure_atr"
    else:
        try:
            idx = df.index.get_loc(day)
...
def manage_position_day(
    ...
    # V2.2 max holding period guard. Useful for avoiding forgotten zombie positions.
    ...
    # V2.2 failed-breakout exit: if a breakout quickly loses its breakout level,
    # recycle capital instead of waiting for a full ATR stop.
    if (
        cfg.enable_failed_breakout_exit
        and str(pos.get("setup_type", "")).lower() == "breakout"
    ...
    # V2.2 time stop: strong breakouts should start working. If not, avoid dead money.
...
def make_variants(base: StrategyConfig) -> List[StrategyConfig]:
    """Compare the old v2.1b breakout-only benchmark against v2.2 ideas."""
    v21b = replace(
        base,
    ...
    return [
        v21b,
        replace(base, name="v2_2_trend_rider"),
        replace(base, name="v2_2_no_weak", allow_weak=False),
        replace(base, name="v2_2_no_partial_trend_rider", partial_fraction=0.0),
        replace(base, name="v2_2_later_partial", partial_r=2.2, partial_pct=0.15, partial_fraction=0.25),
        replace(base, name="v2_2_wider_trail", trail_mult_before_5pct=4.0, trail_mult_after_5pct=3.2),
        replace(base, name="v2_2_no_failfast_or_time_stop", enable_time_stop=False, enable_failed_breakout_exit=False, max_holding_days=0),
        replace(base, name="v2_2_stronger_quality", breakout_min_score=84, breakout_min_volume_ratio=1.35, breakout_min_close_location=0.70, breakout_min_rs_score=20, max_signals_per_scan=3),
    ]
...
def build_config(args: argparse.Namespace) -> StrategyConfig:
    return StrategyConfig(
        ...
        enable_time_stop=args.time_stop_days > 0,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Daily OHLC backtest and walk-forward engine for v2.2 breakout trend-rider strategy.")
    ...
    parser.add_argument("--name", default="v2_2_trend_rider")
    ...
    parser.add_argument("--breakout-min-score", type=int, default=80)
    parser.add_argument("--pullback-min-score", type=int, default=999)
    parser.add_argument("--weak-min-score", type=int, default=86)
    parser.add_argument("--max-signals-per-scan", type=int, default=4)

    parser.add_argument("--allow-pullbacks", action="store_true")
    parser.add_argument("--breakout-min-volume-ratio", type=float, default=1.25)
    parser.add_argument("--pullback-min-volume-ratio", type=float, default=0.30)
    parser.add_argument("--breakout-max-rsi", type=float, default=80.0)
    parser.add_argument("--breakout-max-daily-move-pct", type=float, default=6.5)
    ...
    parser.add_argument("--breakout-min-close-location", type=float, default=0.65)
    parser.add_argument("--breakout-min-rs-score", type=int, default=16)
    parser.add_argument("--no-breakout-ma100-filter", action="store_true")
    parser.add_argument("--no-breakout-ma20-slope-filter", action="store_true")
    parser.add_argument("--breakout-min-stock-ret-20", type=float, default=0.00)
    parser.add_argument("--breakout-min-stock-ret-63", type=float, default=0.00)
    parser.add_argument("--breakout-min-rel-20-spy", type=float, default=0.00)
    parser.add_argument("--breakout-min-rel-63-spy", type=float, default=-0.02)
    parser.add_argument("--breakout-min-rel-20-qqq", type=float, default=-0.03)
    parser.add_argument("--no-55day-or-strong-rs-filter", action="store_true")
    parser.add_argument("--breakout-strong-rs-20-spy", type=float, default=0.06)
    parser.add_argument("--breakout-max-base-range-pct", type=float, default=0.35)
    parser.add_argument("--breakout-max-extension-ma20", type=float, default=0.12)
    parser.add_argument("--breakout-max-extension-atr", type=float, default=1.60)

    parser.add_argument("--breakout-atr-stop-mult", type=float, default=2.00)
    parser.add_argument("--pullback-atr-stop-mult", type=float, default=1.80)
    parser.add_argument("--breakeven-r", type=float, default=1.10)
    parser.add_argument("--partial-r", type=float, default=1.80)
    parser.add_argument("--partial-pct", type=float, default=0.12)
    parser.add_argument("--partial-fraction", type=float, default=0.33)
    parser.add_argument("--trail-mult-before-5pct", type=float, default=3.40)
    parser.add_argument("--trail-mult-after-5pct", type=float, default=2.80)
    parser.add_argument("--trail-tighten-r", type=float, default=2.0)
    parser.add_argument("--trail-tighten-pct", type=float, default=0.12)
    parser.add_argument("--time-stop-days", type=int, default=7)
    parser.add_argument("--time-stop-min-r", type=float, default=0.25)
    parser.add_argument("--no-failed-breakout-exit", action="store_true")
